...
const server=http.createServer((req,res)=>{
 ...
 if(apiPath==='/sync/health'&&req.method==='GET'){
  return json(res,200,{
   ok:true,
   service:SERVICE_ID,
   version:VERSION,
   ...
   maxOnlineDevices:MAX_ONLINE_DEVICES,
   time:Date.now()
  });
 }

 if(apiPath==='/sync/info'&&req.method==='GET'){
  return json(res,200,{
   ok:true,
   service:SERVICE_ID,
   version:VERSION,
 ...
 if(apiPath==='/sync/create'&&req.method==='POST'){
 ...
 if(apiPath==='/sync/join'&&req.method==='POST'){
 ...
 if(apiPath==='/sync/events'&&req.method==='GET'){
 ...
 if(apiPath==='/sync/op'&&req.method==='POST'){
 ...
 if(apiPath.startsWith('/sync/')){
  return json(res,405,{
   ok:false,
   service:SERVICE_ID,
   message:'Endpoint Sync ada, tetapi method HTTP tidak sesuai.',
   method:req.method,
   path:apiPath
  });
 }
...
server.listen(PORT,'0.0.0.0',()=>{
 ...
 console.log('================================================');
 console.log('BUKA APLIKASI DARI ALAMAT INI:');
 console.log('Komputer server : http://localhost:'+PORT+'/dashboard.html');
 const nets=os.networkInterfaces();
 for(const list of Object.values(nets)){
  for(const net of list||[]){
   if(net.family==='IPv4'&&!net.internal){
    console.log('Device lain    : http://'+net.address+':'+PORT+'/dashboard.html');
   }
  }
 }
 console.log('------------------------------------------------');
 console.log('Health API      : http://localhost:'+PORT+'/sync/health');
 console.log('================================================');
 console.log('Satu grup mendukung sampai '+MAX_ONLINE_DEVICES+' device online unik.');
 console.log('Jika pernah HTTP 404, jangan pakai Live Server :5500 sebagai Sync Server.\n');
});
